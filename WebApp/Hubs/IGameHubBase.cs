...
// TODO: Convert this interface into an abstract class 
// TODO: Convert this interface into an abstract class 
public interface IGameHubBase<TGame, TPlayer> where TGame : GameBase where TPlayer : Player
{
    /// <summary>
    ...
    /// <summary>
    /// [LOBBY --> HUB] Client orders the <see cref="GameHub{T,T}"/> to create a new game in the <see cref="GameBase"/> list 
    /// and notify all other <see cref="GameHub{T}"/> clients. Furthermore, the 'GameBase' value is set in the <see cref="HubCallerContext.Items"/>
    /// storage
    /// </summary>
    /// <param name="gameList">
    ///		The STATIC list in an implementation of <see cref="IExposeMethods{TGame, TPlayer}"/> 
    ///		containing all the <typeparamref name="TGame"/> instances in that Hub
    /// </param>
    Task CreateNewGame(List<TGame> gameList);

    /// <summary>
    /// [LOBBY --> HUB] The <see cref="GameHub{T,T}"/> adds a new instance of a <typeparamref name="TPlayer"/> with Title = <paramref name="username"/>
    ...
    ///		The STATIC dictionary in an implementation of <see cref="IExposeMethods{TGame, TPlayer}"/> 
    ...
    /// <summary>
    /// [CLIENT --> HUB] When a user clicks on an available <see cref="GameBase"/> in the <see cref="Components.Applets.GameLobby"/>,
    /// the user's client sends the <see cref="GameBase.Title"/> property to the <see cref="GameHub{TGame, TPlayer}"/> and the player is assigned 
    /// to the corresponding <typeparamref name="TGame"/>
    /// </summary>
    /// <param name="gameNameId">The Title that identifies the specific <typeparamref name="TGame"/></param>
    /// <param name="gameList">
    ///		The STATIC list in an implementation of <see cref="IExposeMethods{TGame, TPlayer}"/> 
    ///		containing all the <typeparamref name="TGame"/> instances in that Hub
    /// </param>
    ...
    /// <summary>
    /// [HUB --> CLIENT] Calls <see cref="GameBase.Restart"/> and broadcasts that clients should do the same.
    /// </summary>
    /// <returns></returns>
    Task RestartGame();
    Task BeginSetup();
